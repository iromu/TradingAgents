...
During an interactive coding session, a user or agent may need to inspect or modify files **not originally loaded** into
the main context. Manually copying/pasting entire files into the prompt is:
...
**2. Summarizes** or **extracts** only the relevant portions (e.g., function bodies, AST-parsed definitions, or specific
line ranges) if the file is large.
...
- The agent's preprocessor intercepts this command, reads the specified file (or line range), and replaces the command
  with the file content (or trimmed snippet).
...
- **Implementation Steps:**
    ...
    3. Read file text, run a **line-range parser** or **AST-based snippet extractor** (e.g., tree-sitter for
...
- **Common Pitfalls:**
    - Path traversal: agent must validate and reject `@../../../etc/passwd`, absolute paths outside project, and
      malicious symlinks.
    - Large injected files: if file > 4,096 tokens, automatically run a **summarizer sub-routine** to extract only
...
- Shunyu Yao et al., "ReAct: Synergizing Reasoning and Acting in Language Models" (ICLR
