...
### Requirement: Manual Flux streaming in streamWithTemplate

**Reason**: Embabel's createObject() handles streaming internally

**Migration**: Replace all `streamWithTemplate()` calls with direct `.withTemplate().createObject()` chains
